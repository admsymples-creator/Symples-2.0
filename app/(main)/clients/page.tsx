...
/**
 * Redireciona /clients para /[workspaceSlug]/clients baseado no workspace ativo
 */
export default async function ClientsPage() {
  ...
  if (activeWorkspaceIdCookie?.value) {
    const workspaceFromCookie = workspaces.find(
      (workspace) => workspace.id === activeWorkspaceIdCookie.value
    );
  ...
  redirect(`/${workspaceSlug}/clients`);
}
