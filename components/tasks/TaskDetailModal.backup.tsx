...
export function TaskDetailModal({ 
    ...
    const [localAssignee, setLocalAssignee] = useState<{ id: string; name: string; avatar?: string } | null>(task?.assignee ? { id: "", name: task.assignee.name, avatar: task.assignee.avatar } : null);
    ...
    const [shareLinkType, setShareLinkType] = useState<"public" | "private">("private");
    ...
    useEffect(() => {
        ...
        if (open && !isCreateMode && task?.id) {
            ...
            // Inicializar assignee local
            setLocalAssignee(task?.assignee ? { id: "", name: task.assignee.name, avatar: task.assignee.avatar } : null);
    ...
    const handleViewTranscription = async (activityId: string, audioUrl: string) => {
        // Verificar se já tem transcrição salva
        const activity = activities.find(a => a.id === activityId);
        if (activity?.audio?.transcription) {
            // Já tem transcrição, apenas expandir/colapsar visualmente (já está sendo exibida)
            return;
        }

        // Se não tem, transcrever
        setTranscribingActivityId(activityId);

        try {
            // Buscar o blob do áudio pela URL
            ...
            // Salvar transcrição na atividade
                    ...
                    <div className="border-r border-gray-100 p-6 overflow-y-auto custom-scrollbar">
                                    ...
                                    <SelectTrigger className="h-8 px-2 -ml-2 justify-start hover:bg-gray-100 border-0 bg-transparent [&>svg]:hidden">
                                        ...
                                        <img
                                            ...
                                            alt={localAssignee.name}
                                                    ...
                                                    <span className="text-sm">{localAssignee.name}</span>
                                                </div>
                                            ) : (
                        ...
                        <div className="mb-6 group/desc">
                            ...
                            {isEditingDescription || isCreateMode ? (
                                ...
                                <div 
                                    className="min-h-[100px] p-3 rounded-md border border-transparent cursor-pointer prose prose-sm max-w-none text-gray-700"
                                    onClick={() => setIsEditingDescription(true)}
                                >
                                                ...
                                                activities.length === 0 ? (
                                                    ...
                                                    >
                                                        <div className="flex-shrink-0 relative z-10">
                                                            <div className="w-2 h-2 rounded-full bg-gray-300 mt-2 ring-4 ring-gray-50" />
                                                        </div>
                                                        <div className="flex-1 pb-2">
                                                            {/* Origin Activity */}
                                                            {activity.type === "origin" && activity.origin && (
                                                                <div className="flex items-start gap-2">
                                                                    <div className={cn(
                                                                        "w-8 h-8 rounded-full flex items-center justify-center shrink-0",
                                                                        activity.origin.source === "whatsapp" ? "bg-green-100" : "bg-blue-100"
                                                                    )}>
                                                                        {activity.origin.source === "whatsapp" ? (
                                                                    ...
                                                                    <div className="flex-1">
                                                                        <p className="text-gray-700 mb-1">
                                                                            ...
                                                                            <span className="font-semibold text-gray-900">
                                                                                {activity.origin.source === "whatsapp" ? "WhatsApp" : "App Web"}
                                                                            </span>
                                                                        </p>
                                                                        {activity.origin.content && (
                                                                            <div className="bg-white p-2.5 rounded-lg border border-gray-200 mt-1.5 shadow-sm text-gray-600">
                                                                                "{activity.origin.content}"
                                                                            </div>
                                                                        )}
                                                                        <p className="text-[10px] text-gray-400 mt-1">{activity.timestamp}</p>
                                                                    </div>
                                                                </div>
                                                            )}
                                                            
                                                            {/* Other Activities */}
                                                            {activity.type !== "origin" && (
                                                                <>
                                                                        ...
                                                                        <div className="mt-2 space-y-2">
                                                                            <div className="max-w-[240px]">
                                                                                <AudioMessageBubble
                                                                                    ...
                                                                                    isOwnMessage={activity.user === "Você"}
                                                                            ...
                                                                            {activity.audio?.url && (
                                                                                <div className="space-y-2">
                                                                                    <Button
                                                                                        ...
                                                                                        onClick={() => handleViewTranscription(activity.id, activity.audio!.url!)}
                                                                                    ...
                                                                                    >
                                                                                        {transcribingActivityId === activity.id ? (
                                                                                            ...
                                                                                            <>
                                                                                                ...
                                                                                                {activity.audio?.transcription ? "Ver transcrição" : "Gerar transcrição"}
                                                                                            </>
                                                                                        )}
                                                                                    </Button>
                                                                                    {activity.audio?.transcription && (
                                                                    ...
                                                                    {/* Legacy single file display (keep for compatibility) */}
                                                                    {activity.file && !activity.attachedFiles && (
